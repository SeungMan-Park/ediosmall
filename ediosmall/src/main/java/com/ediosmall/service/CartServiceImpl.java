...
public class CartServiceImpl implements CartService {
	...
	public List<CartVOList> list_cart(String mbei_id) throws Exception {
		// TODO Auto-generated method stub
		return cartMapper.list_cart(mbei_id);
	}
    ...
    //{"변수명" : [{},{},{}], "변수명" : "값"} - json 문자열 포맷
    @Override
    public JSONObject getCharData() {//제이슨 오브젝트를 리턴하는 것
    	...
    	/*
         List<CartDTO> items;
		try {
			items = cartMapper.cart_money();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}   //***** 이것만 호출하면 됩니다. 아래는 수동 작업이다.
		*/
        ...
        for (CartDTO dto : items) { //items에 저장된 값을 dto로 반복문을 돌려서 하나씩 저장한다.
            ...
            name.put("v", dto.getPdtei_name()); //name변수에 dto에 저장된 상품의 이름을 v라고 저장한다.
            
            JSONObject money = new JSONObject(); //json오브젝트 객체를 생성.   {"v" : 15000}
            money.put("v", dto.getCart_amount()); //name변수에 dto에 저장된 금액을 v라고 저장한다.
